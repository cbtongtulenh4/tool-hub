# -*- coding: utf-8 -*-
# Full demo: PySide6 + qfluentwidgets (tối giản icon, căn font)
...
class DropArea(QFrame):
    """Ô kéo-thả + click chọn file – tối giản icon, tập trung nội dung."""
    clicked = Signal()
    ...
    def __init__(self, text, parent=None):
        ...
        ttl.setStyleSheet("SubtitleLabel{font-weight:700;}")
        lay.addWidget(ttl)
...
class AutoCommentPage(QWidget):
    """Trang 'Bình luận tự động' – tối giản icon, chuẩn font."""
    def __init__(self, parent=None):
        super().__init__(parent)

        # ===== Global font styles (áp cho toàn page) =====
        self.setStyleSheet("""
        ...
        headRow = QHBoxLayout()
        titleTbl = TitleLabel("Danh sách mục tiêu")  # Không icon
        ...
        rowHdr = QHBoxLayout()
        rowHdr.addWidget(SubtitleLabel("Chọn chế độ bình luận"))  # Không icon
        rowHdr.addStretch(1)
        ...
        topC = QHBoxLayout()
        topC.addWidget(SubtitleLabel("Nội dung"))  # Không icon
        topC.addStretch(1)
        self.aiBtn = PushButton("Sử dụng AI")      # Không icon để gọn
        ...
        mediaRow.addWidget(SubtitleLabel("Chọn ảnh/video"))  # Không icon
        ...
        hRow = QHBoxLayout()
        hRow.addWidget(SubtitleLabel("Lịch sử"))  # Không icon
        hRow.addStretch(1)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Dùng TitleLabel để tiêu đề to, rõ
